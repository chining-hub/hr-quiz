...
def text_to_multiline_svg(text: str, font_size: int = 22, max_chars_per_line: int = 50) -> str:
    ...
    svg_height = int(len(lines) * line_height + 40)
    svg_width = 850

    svg_content = f'''<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="{svg_height}" viewBox="0 0 {svg_width} {svg_height}" style="background-color: #ffffff;">
    <style>
        .text {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; font-size: {font_size}px; fill: #1f2937; user-select: none; -webkit-user-select: none; }}
    </style>
    <rect width="100%" height="100%" fill="#ffffff"/>
    <text x="20" y="{font_size + 10}" class="text">'''
    
    for i, line in enumerate(lines):
        escaped_line = line.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace('"', "&quot;")
        if i == 0:
            svg_content += escaped_line
        else:
            dy = line_height
            svg_content += f'''<tspan x="20" dy="{dy}">{escaped_line}</tspan>'''
            
    svg_content += '''</text></svg>'''
    return svg_content

def render_secure_text(text: str, font_size: int = 22, max_chars_per_line: int = 50):
    """將文字渲染為防拷貝 SVG 圖片顯示"""
    svg_str = text_to_multiline_svg(text, font_size, max_chars_per_line)
    b64 = base64.b64encode(svg_str.encode('utf-8')).decode("utf-8")
    html = f'<img src="data:image/svg+xml;base64,{b64}" style="width:100%; max-width:100%; pointer-events: none; -webkit-touch-callout: none;" />'
    st.markdown(html, unsafe_allow_html=True)

# =========================================================================
# 🚀 Streamlit 主介面與一題一題作答邏輯
# =========================================================================
st.set_page_config(page_title="線上測驗系統", page_icon="📝", layout="centered")
...
if not st.session_state.logged_in:
    ...
    with st.form("login_form"):
        ...
        exam_type = st.selectbox("選擇測驗科目", ["英文測驗", "數學測驗"])
        submit_login = st.form_submit_button("開始測驗")
        
        if submit_login:
            if name.strip() and dept.strip():
                ...
                save_new_test(test_id, name, dept, exam_type)
...
# -------------------------------------------------------------------------
    ...
    quiz_data = ENGLISH_QUIZ_DATA if exam_type == "英文測驗" else MATH_QUIZ_DATA
    total_questions = len(quiz_data)
    
    st.sidebar.title(f"考生：{st.session_state.name}")
    st.sidebar.write(f"部門：{st.session_state.dept}")
    st.sidebar.write(f"科目：{exam_type}")
    st.sidebar.markdown("---")
    ...
    # 顯示圖片（若有）
    if "image_key" in current_q:
        if current_q["image_key"] == "attachment1":
            display_quiz_image("attachment1.jpg", "閱讀測驗圖表 1")
        elif current_q["image_key"] == "attachment2":
            display_quiz_image("attachment2.jpg", "閱讀測驗圖表 2")
            
    # 渲染題目（使用 SVG 防偷看模組）
    render_secure_text(current_q['question'], font_size=20, max_chars_per_line=45)
    ...
    with col3:
        if current_idx < total_questions - 1:
            ...
            if st.button("✅ 確認交卷", type="primary", use_container_width=True):
                ...
                details_list = []
                for q in quiz_data:
                    ans = st.session_state.user_answers.get(q["id"], "未作答")
                    is_correct = (ans == q["answer"])
                    if is_correct:
                        correct_count += 1
                    details_list.append({"id": q["id"], "user_ans": ans, "correct": is_correct})
                
                score = round((correct_count / total_questions) * 100, 2)
                duration_sec = int((datetime.now() - st.session_state.start_time).total_seconds())
...
# -------------------------------------------------------------------------
    ...
    correct_count = sum(1 for q in quiz_data if st.session_state.user_answers.get(q["id"]) == q["answer"])
    total_questions = len(quiz_data)
    score = round((correct_count / total_questions) * 100, 2)
    
    st.write(f"### 考生姓名：{st.session_state.name}")
    st.write(f"### 所屬部門：{st.session_state.dept}")
